...
// ChunkText splits a long text into chunks of approx maxChars size, with overlap.
func ChunkText(text string, maxChars int, overlap int) []string {
	...
	for i := 0; i < n; {
		...
		if i >= end {
			i = end // avoid infinite loop
		}
	}
	return chunks
}

// GetGeminiEmbedding calls the Gemini API to get a 768-dimension vector embedding for the given text.
...
// FormatVectorAsString converts a float32 slice to PostgreSQL pgvector string format "[v1,v2,v3,...]"
