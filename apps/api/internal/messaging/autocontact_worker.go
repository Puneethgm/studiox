...
func (w *AutoContactWorker) processItem(ctx context.Context, it leads.OutboxItem) error {
    ...
    // Build initial message (Option B)
    first := firstName(l.Name)
    body := fmt.Sprintf("Hi %s, we saw your interest in %s. I’m from %s — can I help answer any questions?", first, l.CampaignName, l.StudioName)
    ...
    // Schedule follow-ups: 40s, 2h, 12h
