...
import (
	...
	_ "github.com/projectx/api/docs" // swagger spec, generated by `make swagger`
...
// @description				Auth is a JWT stored in an HTTP-only cookie (set by POST /api/v1/auth/login).
// @description				When calling protected endpoints from a tool like Swagger UI or curl, log in first and reuse the returned cookie.
// @contact.name				Project-X support
...
func main() {
	...
	// with CRM-doc parsing) — reuses the platform Claude client above; no
	// platform-wide Groq client exists (Groq is always a studio's own BYO
	// key elsewhere in this codebase), so Groq here only works via an
	...
	// Interactive API docs — generated from handler comments via `make swagger`.
	// Not exposed through nginx in production unless explicitly proxied; safe to
	// leave mounted since it only reveals route shapes, not secrets.
